...
#     Basic Modules      #
# -*-*-*-*-*-*-*-*-*-*-* #
import requests, pickle, os, json, yaml, time, codecs, base64, sys, re
import urllib.parse
import pandas as pd
import numpy as np
...
class GoogleSheetsAPI(GoogleAPI):
    ...
    def result_to_df(self, result, has_index=True, has_headers=True):
    ...
    def _a1_to_rowcol(self, a1: str):
        """Convert A1 notation to zero-based row, column indices."""
        match = re.match(r"([A-Z]+)(\d+)", a1, re.I)
        if not match:
            raise ValueError(f"Invalid A1 notation: {a1}")
        col_letters, row_number = match.groups()
        
        # Convert column letters to number (A=1, B=2, ..., Z=26, AA=27, AB=28...)
        col_number = 0
        for c in col_letters.upper():
            col_number = col_number * 26 + (ord(c) - ord('A') + 1)
    ...
    def update_range(self, spreadsheet_id: str, tab_name: str, range_name: str, values: List) -> bool:
        ...
        try:
            ...
            sheets = {s['properties']['title']: s['properties'] for s in spreadsheet['sheets']}
            ...
            props = sheets[tab_name]
            sheet_id, row_count, col_count = props['sheetId'], props['gridProperties']['rowCount'], props['gridProperties']['columnCount']

            # Check required size
            _, end_a1 = range_name.split(':')  # take the last cell
            last_row, last_col = self._a1_to_rowcol(end_a1)

            requests_body = []
            if last_row > row_count:
                requests_body.append({"appendDimension": {"sheetId": sheet_id, "dimension": "ROWS", "length": last_row - row_count}})
            if last_col > col_count:
                requests_body.append({"appendDimension": {"sheetId": sheet_id, "dimension": "COLUMNS", "length": last_col - col_count}})
            if requests_body:
                self.service.spreadsheets().batchUpdate(spreadsheetId=spreadsheet_id, body={"requests": requests_body}).execute()


            # Update range after ensuring size
            ...
            self.mk1.logging.logger.info(
                f"(GoogleSheetsAPI.update_range) Updated {result.get('updatedCells', 0)} cells in {tab_name}!{range_name}"
            )
            return True
    ...
    @retry(exceptions=requests.RequestException, tries=10, delay=2, jitter=(0, 2))
    def write_df_to_tab2(self, spreadsheet_id: str, tab_name: str, df: pd.DataFrame):
        """
        Write a DataFrame to a specific tab in a Google Sheet.
        Replaces the entire tab content with df.
        """
        ...
        # Correct keys for values:batchUpdate
        return {
    ...
    def ensure_sheet_exists(self, spreadsheet_id: str, sheet_name: str):
        ...
        response = requests.get(url, headers=self.auth_header)
        response.raise_for_status()
        sheets = [s["properties"]["title"] for s in response.json()["sheets"]]
        ...
        if sheet_name not in sheets:
            ...
            url = f"https://sheets.googleapis.com/v4/spreadsheets/{spreadsheet_id}:batchUpdate"
            resp = requests.post(url, headers=self.auth_header, json=body)
            resp.raise_for_status()
            print(f"✅ Created new sheet: {sheet_name}")


    @retry(exceptions=requests.RequestException, tries=10, delay=2, jitter=(0, 2))
    def format_sheet_tab(self, spreadsheet_id: str, tab_name: str, df: pd.DataFrame, start_col_idx : int = 0):
        """
        Apply formatting to a Google Sheet tab:
        - Center all cells
        - Bold header
        ...
        try:
                ...
                # 3️⃣ Add borders
                {
                    "updateBorders": {
                        "range": {
                            ...
                            "endColumnIndex"   : start_col_idx + num_cols
                        },
                        "top": {"style": "SOLID"},
                        "bottom": {"style": "SOLID"},
                        "left": {"style": "SOLID"},
            ...
            # # 5️⃣ Adjust column widths based on header length
            ...
            #                 "sheetId": sheet_id,
            #                 "dimension": "COLUMNS",
            #                 "startIndex": col_idx + start_col_idx,
            #                 "endIndex": col_idx + start_col_idx +1,
            #             },
            #             "properties": {
            #                 "pixelSize": max(100, len(header) * 10)  # adjust 10 for scaling
            #             },
            #             "fields": "pixelSize"
            #         }
            #     })
